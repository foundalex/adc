function [y_array, y_array_int, DetM_2x2_array, DetM_2x2_array_int, Det2x2_mult1_abs_max, Det2x2_mult2_abs_max, Det2x2_sum_abs_max, ...
    ...
    ] = least_mean_square(adc_input, adc_input_int, yri_cut, yri_cut_int, sim_options)
...
x3_int = cast(zeros(sim_options.Size_matrix,sim_options.Size_matrix), sim_options.int_size); 

% функция поиска определителя 2x2
Det2x2_mult1_abs_max = cast(zeros(sim_options.num_det2x2,1), sim_options.int_size); % первые 10 умножителей1
Det2x2_mult2_abs_max = cast(zeros(sim_options.num_det2x2,1), sim_options.int_size); % первые 10 умножителей2
Det2x2_sum_abs_max = cast(zeros(sim_options.num_det2x2,1), sim_options.int_size); % первые 10 сумматоров
...
DetM_2x2_array_int(1:sim_options.num_det2x2) = DetM_2x2_int_double;
DetM_2x2_array(1:sim_options.num_det2x2) = DetM_2x2;
bb = bb + 10;
...
for j = 1:length(yri_cut(:,1))-2*sim_options.Size_matrix
	...
	DetM_2x2_array_int(bb+1:bb+sim_options.num_det2x2) = DetM_2x2_int_double;
	DetM_2x2_array(bb+1:bb+sim_options.num_det2x2) = DetM_2x2;
	bb = bb + sim_options.num_det2x2;
	%%
	if (det_matlab(tt) == 0)
		det_matlab(tt) = 1;
	end
