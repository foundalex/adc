function [data_outd, data_out] = filter_transversal(input_data_double, coeff_double, input_data, coeff)
    ...
    % data_out = fi((mult1 + mult2 + mult3 + mult4 + mult5),1,67,0); % fi(1,67,0)
    % y_out_int =  + fi(www1_int(k) * fi(adc_input_int(j+k,z),1,12,0),1,82,66); % (стр 5, (13)) % fi(1,70,55) * fi(1,12,11) = fi(1,82,66)
