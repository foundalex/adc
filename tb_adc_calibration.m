...
for num = 0:num_cycles
    ...
    freq = freq + step;              % frequency of fundamental tone
    ...
    adc_input(:,1) = s(1:M*Inter:end);
    for i = 1:M-1
        adc_input(:,i+1) = s(i*Inter+1:M*Inter:end);
        indexx(i) = (i*Inter+1);
    end
    ...
    [y_array, error_out] = least_mean_squares(adc_input, yri_cut, M, N);

    % create main signal after LS algorithm (switch after sub-adc)
    x_after_adc = zeros(length(y_array)*M,1);
    for i = 1:M
        if i == 1
            x_after_adc(i:M:end) = yri_cut(1:length(y_array));
        else
            x_after_adc(i:M:end) = y_array(:,i-1);
        end
    end
    ...
    norm_freq(num+1) = freq/(Fs/(Inter)/M);
...
function adc_input_skew = time_skew_func(time_skew, s, indexx, Inter, M) 
    adc_input_skew = s(indexx + time_skew*Inter:M*Inter:end);
end